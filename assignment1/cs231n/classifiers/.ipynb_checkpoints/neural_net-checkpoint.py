...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        data_loss =np.mean(-np.log(softmax_out[range(N),list(y)]))
        ...
        dW2 = h_out.T.dot(dScores) + 2*reg*W2
        dh_out = np.dot(dScores,W2.T)
        db2 = np.sum(dScores,axis=0)
        
        drelu = np.zeros(h_out.shape)
        drelu[h_out>0] = 1
        drelu*=dh_out
    ...
    def train(self, X, y, X_val, y_val,
        ...
        """
        num_train = X.shape[0]
        iterations_per_epoch = max(num_train / batch_size, 1) # 这里会导致最后只有一个元素在list
